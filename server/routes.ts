...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  app.use('/api', (req, res, next) => {
    if (req.originalUrl.includes('upload-binary')) {
      console.log(`🔍 BINARY UPLOAD: ${req.method} ${req.originalUrl} - Content-Type: ${req.headers['content-type']}`);
  ...
  app.use('/api/videos', (req, res, next) => {
    console.log(`🔍 GLOBAL: ${req.method} ${req.originalUrl} - Content-Type: ${req.headers['content-type']}`);
    console.log(`🔍 GLOBAL: Body keys: ${req.body ? Object.keys(req.body) : 'no body'}`);
    next();
  });
    ...
    try {
      ...
      if (videoData.startsWith('http://') || videoData.startsWith('https://')) {
        try {
          ...
          if (!response.ok) {
            console.error(`Failed to fetch video from Bunny.net: ${response.status} ${response.statusText}`);
            return res.status(404).json({ message: "Video not available for streaming" });
          }
      ...
      if (videoData.startsWith('blob:')) {
        return res.status(404).json({ message: "Video no longer available (legacy format)" });
      }
      ...
      if (commaIndex === -1) {
        return res.status(400).json({ message: "Invalid video data format - no comma separator" });
      }
      
      const header = videoData.substring(0, commaIndex);
      const content = videoData.substring(commaIndex + 1);
      const originalMimeType = header.match(/data:([^;]+)/)?.[1] || 'video/webm';
      ...
      if (originalMimeType === 'video/mp4') {
        if (header.includes('codecs="avc1.42001E, mp4a.40.2"')) {
          ...
          mimeType = 'video/mp4';
          console.log('Detected fragmented MP4 with H.264/AAC - optimal compatibility');
        } else if (header.includes('codecs=avc1,opus') || header.includes('codecs=avc1,mp4a')) {
  ...
  app.post("/api/videos", isAuthenticated, async (req: any, res) => {
    try {
      ...
      console.log('🔍 MAIN VIDEO ENDPOINT: Content-Type:', req.headers['content-type']);
      console.log('🔍 MAIN VIDEO ENDPOINT: Body keys:', Object.keys(req.body || {}));
      ...
      if (videoData.videoUrl && videoData.videoUrl.startsWith('data:video/')) {
        ...
        const tempFilePath = join('/tmp', `upload_${videoId}_${Date.now()}${fileExtension}`);
        console.log(`Detected MIME type: ${detectedMimeType}, using extension: ${fileExtension}`);
        
        // Save temporary file
        await writeFile(tempFilePath, videoBuffer);
        console.log(`Saved temporary video file: ${tempFilePath} (${videoBuffer.length} bytes)`);
        
        // Create persistent copy BEFORE processing
        const persistentFilePath = join('/tmp', `video_${videoId}_${Date.now()}${fileExtension}`);
        ...
        videoProcessor.processVideo(persistentFilePath, videoId, videoData.duration)
          ...
          .catch(async (error) => {
            ...
            try {
              ...
              console.log(`✅ DATABASE: Video ${videoId} marked as failed`);
            } catch (dbError) {
              console.error(`Failed to update video ${videoId} status to failed:`, dbError);
            }
          });
