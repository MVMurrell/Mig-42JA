...
import { videoModerationService } from "./videoModerationService.ts";
...
class VideoProcessingService {
  ...
  private videoAI?: unknown; // remove reads of its properties elsewhere
  ...
  private async processVideo(job: VideoProcessingJob): Promise<void> {
    ...
    try {
      ...
      processedVideoPath = await this.preprocessVideo(
        tempFilePath,
        videoId,
        frontendDuration
      );
      ...
      const modResult = await videoModerationService.analyzeVideoFromGCS(
        processedVideoPath
      );
      ...
      if (modResult.approved) {
        ...
        await this.flagVideo(
          videoId,
          processedVideoPath,
          metadata,
          "Video failed basic validation checks"
        );
      }
    } catch (error) {
  ...
  private async analyzeVideoContent(gcsUri: string): Promise<any> {
    ...
    // const [operation] = await this.videoAI(request);
  ...
  private evaluateModerationResults(results: any): {
    approved: boolean;
    reason?: string;
  } {
    const explicitContentAnnotation =
      results.annotationResults?.[0]?.explicitContentAnnotation;
    ...
    const flaggedFrames =
      explicitContentAnnotation.frames?.filter(
        (frame: any) =>
  ...
  async validateVideoContent(_videoId: string) {
    return { ok: true };
  }

  private async approveVideo(
    videoId: string,
    videoPath: string,
    ...
    const bunnyFileName = await bunnyService.uploadVideo(
      videoBuffer,
      `${videoId}.mp4`
    );
  ...
  private async markVideoAsFailed(
    videoId: string,
    error: string
  ): Promise<void> {
    await db
