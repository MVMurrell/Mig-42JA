...
export class VideoModerationService {
  ...
  constructor() {
    try {
      ...
      if (!contentModerationCredentials) {
        throw new Error('CONTENT_MODERATION_WORKER_JUN_26_2025 credentials not found - required for video content analysis');
      }
      ...
      console.log('🔧 VIDEO-MODERATION: Initialized with CONTENT_MODERATION_WORKER credentials for video analysis');
    } catch (error) {
      console.error('❌ VIDEO-MODERATION: Failed to initialize Video Intelligence Service:', error);
      throw error;
    }
  }
  ...
  async analyzeVideoFromGCS(gcsUri: string): Promise<VideoModerationResult> {
    try {
      console.log(`🎬 VIDEO-MODERATION: Starting video content analysis for: ${gcsUri}`);
      ...
      console.log(`⏳ VIDEO-MODERATION: Video analysis operation started, waiting for completion...`);
  ...
  private evaluateVideoResults(result: any): VideoModerationResult {
    ...
    if (!annotationResults) {
      console.log(`⚠️ VIDEO-MODERATION: No annotation results found - approving by default`);
      return {
        approved: true,
    ...
    if (explicitAnnotation?.frames) {
      ...
      for (const frame of explicitAnnotation.frames) {
        ...
        if (['VERY_LIKELY'].includes(likelihood)) {
          ...
          console.log(`🚨 VIDEO-MODERATION: Explicit content detected in frame - likelihood: ${likelihood}`);
          break;
        } else if (['LIKELY'].includes(likelihood)) {
          console.log(`⚠️ VIDEO-MODERATION: Possible content detected but allowing due to false positive reduction - likelihood: ${likelihood}`);
        }
      }
    }

    // Extract labels for additional context
    const labels = annotationResults.labelAnnotations?.map((label: any) => label.entity.description) || [];
    
    console.log(`📊 VIDEO-MODERATION: Analysis summary - Explicit frames: ${explicitContentFrames}, Inappropriate: ${hasInappropriateContent}, Labels: ${labels.slice(0, 5).join(', ')}`);
