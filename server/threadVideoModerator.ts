...
export class ThreadVideoModerator {
  ...
  constructor() {
    try {
      ...
      console.log(
        "Thread Video Moderator initialized with new secure credentials"
      );
    } catch (error) {
  ...
  async processThreadVideo(
    ...
    try {
      console.log(
        `🔒 SECURE THREAD VIDEO: Processing message ${options.messageId}`
      );
      console.log(
        `📋 SECURITY PIPELINE: GCS → AI Moderation → Bunny Upload (only if approved)`
      );
      ...
      try {
        gcsUri = await this.uploadToGCS(
          `thread-message-${options.messageId}`,
          tempFilePath
        );
        console.log(
          `📤 Thread message ${options.messageId} uploaded to GCS: ${gcsUri}`
        );
        ...
        console.error(
          `🚨 CRITICAL SECURITY: GCS upload failed for message ${options.messageId}:`,
          gcsError
        );
      ...
      if (finalStatus === "approved") {
        console.log(
          `✅ Thread message ${options.messageId} APPROVED - Uploading to Bunny.net`
        );

        const videoBuffer = await readFile(tempFilePath);
        bunnyVideoId = await bunnyService.uploadVideo(
          videoBuffer,
          `thread-message-${options.messageId}.mp4`
        );
        ...
        console.log(
          `✅ Thread message ${options.messageId} uploaded to Bunny: ${bunnyVideoId}`
        );
      } else {
        console.log(
          `❌ Thread message ${
      ...
      if (finalStatus === "approved" && bunnyVideoId) {
        await storage.updateThreadMessageVideo(options.messageId, {
          ...
          extractedKeywords: audioModerationResult.keywords
            ? JSON.stringify(audioModerationResult.keywords)
            : undefined,
        });
      }
      ...
      await storage.updateThreadMessageModerationResults(
        options.messageId,
        JSON.stringify(moderationData)
      );

      console.log(
        `🔒 Thread message ${options.messageId} secure processing completed: ${finalStatus}`
      );
      ...
      try {
        ...
        console.log(
          `🧹 Temporary file cleaned up for thread message ${options.messageId}`
        );
        // Note: GCS files are preserved for debugging and appeal processes
      } catch (cleanupError) {
        console.warn(
          `Cleanup warning for thread message ${options.messageId}:`,
          cleanupError
        );
      }
    } catch (error) {
      console.error(
        `❌ CRITICAL: Thread video processing failed for ${options.messageId}:`,
        error
      );
  ...
  private async moderateVideoContent(
    ...
    try {
      console.log(
        `🛡️ COMPREHENSIVE VIDEO AI: Starting enhanced moderation for thread message ${messageId}`
      );
      ...
      console.log(
        `🎥 THREAD VIDEO AI: Analyzing ${gcsUri} with comprehensive detection`
      );
      ...
      console.log(
        `🎥 THREAD VIDEO AI: Analysis operation started for message ${messageId}`
      );
      ...
      if (!annotations) {
        console.log(
          `🚫 THREAD VIDEO AI: CRITICAL SECURITY FAILURE - No analysis results for message ${messageId}, REJECTING`
        );
        return {
          passed: false,
          reason:
            "Video Intelligence API analysis failed - content rejected for security",
        };
      }
      ...
      if (explicitAnnotation?.frames) {
        console.log(
          `🎥 THREAD VIDEO AI: Found ${explicitAnnotation.frames.length} explicit content frames`
        );

        for (const frame of explicitAnnotation.frames) {
          const likelihood = frame.pornographyLikelihood;
          console.log(
            `🎥 THREAD VIDEO AI: Frame explicit likelihood: ${likelihood}`
          );

          if (["LIKELY", "VERY_LIKELY"].includes(String(likelihood) || "")) {
            console.log(
              `🚫 THREAD VIDEO AI: REJECTED - Explicit content detected with likelihood: ${likelihood}`
            );
            return {
              passed: false,
      ...
      console.log(
        `🎥 THREAD VIDEO AI: Found ${objectAnnotations.length} tracked objects`
      );
      ...
      for (const objectAnnotation of objectAnnotations) {
        ...
        if (
          description.includes("finger") ||
          description.includes("hand gesture") ||
          description.includes("middle finger")
        ) {
          console.log(
            `🚫 THREAD VIDEO AI: REJECTED - Inappropriate hand gesture detected: ${description}`
          );
          return {
            passed: false,
      ...
      console.log(
        `🎥 THREAD VIDEO AI: Found ${personDetections.length} person detection results`
      );
      ...
      for (const detection of personDetections) {
        ...
        for (const track of tracks) {
          ...
          for (const attribute of attributes) {
            ...
            console.log(
              `🎥 THREAD VIDEO AI: Person attribute detected: ${name}`
            );

            // Check for gesture-related attributes
            if (
              name.includes("gesture") ||
              name.includes("inappropriate") ||
              name.includes("offensive")
            ) {
              console.log(
                `🚫 THREAD VIDEO AI: REJECTED - Inappropriate gesture attribute: ${name}`
              );
              return {
                passed: false,
      ...
      console.log(
        `✅ THREAD VIDEO AI: Comprehensive analysis completed - No inappropriate content detected, APPROVED for message ${messageId}`
      );
      return { passed: true };
    } catch (error) {
      console.error(
        `❌ THREAD VIDEO AI: Analysis failed for message ${messageId}:`,
        error
      );
      // Fail closed - reject on moderation errors for security
      console.log(
        `🚫 THREAD VIDEO AI: REJECTED due to analysis failure - fail-closed security for message ${messageId}`
      );
      return {
        passed: false,
        reason:
          "Video moderation service error - content rejected for security",
      };
    }
  }
  ...
  private async moderateAudioContent(
    ...
    try {
      console.log(
        `🎵 CRITICAL SECURITY: Audio moderation starting for thread message ${messageId}`
      );
      ...
      console.log(
        `🎵 THREAD AUDIO: Processing video from correct GCS path: ${gcsUri}`
      );
      ...
      try {
        ...
        if (!result.success) {
          console.error(
            `🚨 SECURITY VIOLATION: Audio processing failed for thread message ${messageId}: ${result.error} - REJECTING`
          );
          return {
            passed: false,
            reason:
              result.error ||
              "SECURITY: Audio processing failed - content rejected for safety",
          };
        }
        ...
        if (result.moderationStatus === "failed") {
          console.log(
            `🚫 INAPPROPRIATE CONTENT DETECTED in thread message ${messageId}: ${result.flagReason}`
          );
          return {
            passed: false,
        ...
        console.log(
          `✅ Audio moderation passed for thread message ${messageId}`
        );
        return {
          passed: true,
          transcription: result.transcription,
        ...
        console.error(
          `🚨 SECURITY VIOLATION: Audio processing error for thread message ${messageId}:`,
          audioError
        );
        return {
          passed: false,
      ...
      console.error(
        `🚨 SECURITY VIOLATION: Error in audio moderation for thread message ${messageId}:`,
        error
      );
      return {
        passed: false,
        reason:
          "SECURITY: Audio moderation failed - content rejected for safety",
      };
    }
  }
  ...
  private async extractAudio(
    ...
    return new Promise((resolve) => {
      const ffmpegArgs = [
        ...
        "-ar",
        "16000", // Sample rate for speech recognition
        "-ac",
        "1", // Mono channel
        "-y", // Overwrite output
        audioPath,
  ...
  private static detectProfanity(text: string): string[] {
  ...
  public static detectProfanityImpl(text: string): string[] {
    return ThreadVideoModerator.detectProfanity(text);
  }
